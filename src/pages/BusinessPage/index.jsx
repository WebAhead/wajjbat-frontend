...
export default function BusinessPage(props) {
  ...
  useEffect(() => {
    async function getImages() {
      const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/businesses/${props.match.params.id}`);
    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      <nav className="business-page-nav">
        <div className="nav-items">
          <button className="nav-button" onClick={() => setActiveTab('details')}>
            <FormattedMessage id="Details" />
          </button>
          <button className="nav-button" onClick={() => setActiveTab('reviews')}>
